...
const gSelectData = [
  {
    ...
    //WBL 2 Jun 2025 sig: (aT) => aT._totalBytes >= 0.01 * gRoot._totalBytes,
    sig: (aT) => aT._totalBytes >= 0.0 * gRoot._totalBytes,
    sigLabel: () => `\
total >= ${bytesAndPerc(0.0 * gRoot._totalBytes, gRoot._totalBytes)}`
  },
  {
    isDefault: true,
    label: "Total (blocks)",
    ...
    sig: (aT) => aT._totalBlocks >= 0.0 * gRoot._totalBlocks,
    sigLabel: () => `\
total >= ${blocksAndPerc(0.0 * gRoot._totalBlocks, gRoot._totalBlocks)}`
  },
...
((reads <= ${perByte(0.4)}) || (writes <= ${perByte(0.4)}))`
  ...
  // No "Max (avg size bytes)": not interesting, and unclear how to sort.
  {
    ...
    sig: (aT) => aT._atTGmaxBytes >= 0.0 * gRoot._atTGmaxBytes,
    sigLabel: () => `\
at-t-gmax >= ${bytesAndPerc(0.0 * gRoot._atTGmaxBytes, gRoot._atTGmaxBytes)}`
  },
  ...
  // No "At t-gmax (avg size bytes)": not interesting.
  {
    ...
    sig: (aT) => aT._atTEndBytes >= 0.0 * gRoot._atTEndBytes,
    sigLabel: () => `\
at-t-end >= ${bytesAndPerc(0.0 * gRoot._atTEndBytes, gRoot._atTEndBytes)}`
  },
  ...
  // No "At t-end (avg size bytes)": not interesting.
  {
    ...
    sig: (aT) => aT._readsBytes >= 0.0 * gRoot._readsBytes,
    sigLabel: () => `\
reads >= ${bytesAndPerc(0.0 * gRoot._readsBytes, gRoot._readsBytes)}`
  },
  {
...
writes >= ${bytesAndPerc(0.01 * gRoot._writesBytes, gRoot._writesBytes)}`
  },
  {
    ...
    cmpField: "_writesBytes",
    sig: (aT) => aT._writesBytes >= 0.0 * gRoot._writesBytes &&
                 (aT._readsAvgPerByte() >= 1000 ||
                  aT._writesAvgPerByte() >= 1000),
    sigLabel: () => `\
(writes >= ${bytesAndPerc(0.0 * gRoot._writesBytes, gRoot._writesBytes)}) && \
((reads >= ${perByte(1000)}) || (writes >= ${perByte(1000)}))`
  }
